          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(saleAttr, index) in spuSaleAttrList" :key="saleAttr.id">
                ...
                <dd changepirce="0" :class="{ active: saleValue.isChecked == 1 }" v-for="(saleValue, index) in saleAttr.spuSaleAttrValueList" :key="saleValue.id" @click="changeActive(saleValue,saleAttr.spuSaleAttrValueList)">
                  {{ saleValue.saleAttrValueName }}</dd>
              </dl>
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 注意：购物车跳转路由之前，要发一个请求，产品信息通过请求的形式通知服务器，服务器进行存储 -->
...
export default {
  ...
  methods: {
    // 表单元素修改商品个数
    changeSkuNum (e) {
      // 用数据乘数字，可以当做筛选条件，有非数字字符则value为NaN
      const value = e.target.value * 1
      // 如果用户输入的是非法的，或者小于1的，则置1
    ...
    // 加入购物车回调函数
    async addShopCart () {
      // 1.将产品加到数据库，通知服务器，成功或失败的处理
      // 2.服务器成功存储---跳转带参
      // 3.失败要有提示
      // 如何判断成功与失败：dispatch调用了一个函数addOrUpdateShopCart，返回的是一个promise，若我们给这个函数加了返回值，用来判断成功或失败
      try {
        const result = await this.$store.dispatch('addOrUpdateShopCart', { skuId: this.$route.params.skuid, skuNum: this.skuNum })
        // 成功了跳转路由
        // 先将要传的复杂数据（对象）转化为字符串，存放在浏览器
